...
# NOTE: this converts the tendermint private ket format to the eris private key format
...
# curl that baby!
RESULT=`curl --silent -X POST -d "${REQ}" $URL --header "Content-Type:application/json"`

# fetch fields from the json result
...
# save the ABI to a file for use later
echo $ABI > add.abi
...
# we're goingi to need the pubkey
# (this can also be done as a curl request to eris-keys but the signing example above should be sufficiently illustrative)
...
# the --wait flag waits until the transaction is confirmed
...
if [[ "$SUM_GOT" != "$SUM_EXPECTED" ]]; then
	echo "SMART CONTRACT ADDITION FAILED"
	echo "GOT $SUM_GOT"
	echo "EXPECTED $SUM_EXPECTED"
else
	echo "SMART CONTRACT ADDITION SUCCEEDED!"
